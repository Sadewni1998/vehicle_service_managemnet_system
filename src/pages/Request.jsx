...
import { Car, Wrench, CheckCircle, Phone, MapPin, Clock, Navigation } from 'lucide-react'
...
const Request = () => {
      ...
      async (position) => {
        try {
          ...
          const response = await fetch(
            `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=${import.meta.env.VITE_GOOGLE_MAPS_API_KEY || 'YOUR_API_KEY'}`
          )
  ...
  const emergencyTypes = ['Engine Failure', 'Battery Dead', 'Flat Tire', 'Accident', 'Overheating', 'Transmission Issue', 'Electrical Problem', 'Other']
        ...
        <div className="relative z-10 h-full flex items-center justify-center">
          <div className="text-center text-white">
            ...
            <nav className="text-sm">
              <span>Home</span> / <span>Pages</span> / <span className="text-primary-400">Breakdown Service</span>
            </nav>
          </div>
        </div>
      </div>
            ...
            {[
              {
                ...
                description: "Our breakdown service is available 24/7 to help you when you need it most. We understand that breakdowns don't happen on schedule."
              },
              {
                icon: <MapPin className="w-12 h-12 text-primary-600" />,
                title: "Quick Response",
                description: "We provide rapid response to your location with our mobile service units equipped with all necessary tools and parts."
              },
              {
                icon: <Wrench className="w-12 h-12 text-primary-600" />,
                title: "Expert Technicians",
                description: "Our certified technicians are trained to handle all types of vehicle breakdowns and will get you back on the road safely."
              }
            ].map((service, index) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className="text-white text-lg leading-relaxed mb-6">
                When your vehicle breaks down, you need reliable help fast. Our emergency breakdown service 
                provides 24/7 roadside assistance with certified technicians who can diagnose and repair 
                most issues on the spot. We're equipped with mobile service units and genuine parts to 
            ...
            <div className="bg-primary-600 p-8 rounded-lg">
              <h2 className="text-3xl font-bold text-white mb-8 text-center">Request Breakdown Service</h2>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <select
                      className="w-full px-4 py-3 rounded-lg border-0 focus:ring-2 focus:ring-white"
                      defaultValue=""
                      {...register('emergency_type', { required: 'Emergency type is required' })}
                    >
                      <option value="" disabled hidden>Type of Emergency</option>
                      {emergencyTypes.map(type => (
                        <option key={type} value={type.toLowerCase()}>{type}</option>
                      ))}
                    </select>
                    {errors.emergency_type && <p className="text-red-200 text-sm mt-1">{errors.emergency_type.message}</p>}
                  </div>
                </div>
                ...
                <div>
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-3 rounded-lg border-0 focus:ring-2 focus:ring-white"
                    {...register('problem_description', { required: 'Problem description is required' })}
                  />
                  {errors.problem_description && <p className="text-red-200 text-sm mt-1">{errors.problem_description.message}</p>}
                </div>
                ...
                <button
                  ...
                  className="w-full bg-blue-900 text-white font-semibold py-4 px-6 rounded-lg 
                            ...
                            disabled:opacity-50"
                >
                  {isSubmitting ? 'Submitting Request...' : 'Request Emergency Service'}
                </button>
              </form>
            </div>
          </div>
      ...
      <section className="section-padding bg-red-50">
        <div className="container-custom">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-lg">For immediate assistance, call our emergency hotline</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center p-6 bg-white rounded-lg shadow-md">
              ...
              <p className="text-lg font-semibold text-primary-600">Colombo & Suburbs</p>
            ...
            <div className="text-center p-6 bg-white rounded-lg shadow-md">
              ...
              <p className="text-lg font-semibold text-green-600">15-30 Minutes</p>
